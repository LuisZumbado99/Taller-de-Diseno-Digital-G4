module memory (
    ...
    // Volvemos al contador síncrono original
    logic [23:0] cpu_activity = 24'd0;

    assign led_out[15]    = pll_locked;
    assign led_out[14]    = valid;
    assign led_out[13]    = ready;
    assign led_out[12]    = cpu_activity[23];
    ...
    always_comb begin
        ...
        if (valid) begin
            ...
            else                rdata = 32'h0000_0013;
        end
    end

    always_ff @(posedge clk) begin
        if (valid && ready) begin
            cpu_activity <= cpu_activity + 1'b1;
        end
