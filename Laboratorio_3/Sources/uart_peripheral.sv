module uart_peripheral #(
    parameter CLK_FREQ_HZ = 10_000_000, // Ajustado por defecto a los 10MHz del sistema
    ...
    // -------------------------------------------------------------------------
    // Bus handshake (Nativo y seguro para el pipeline del PicoRV32)
    // -------------------------------------------------------------------------
    always_ff @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            mem_ready <= 1'b0;
        end else begin
            // Si el bus inicia un ciclo válido y no hemos respondido aún, activamos ready por UN ciclo
            if (mem_valid && !mem_ready) begin
                mem_ready <= 1'b1;
            end else begin
                // En cuanto el ciclo termina o se cumple el ciclo de ready, se apaga de inmediato
                mem_ready <= 1'b0;
            end
        end
    end
    ...
    // -------------------------------------------------------------------------
    // Lectura combinacional aislada (Solo responde si mem_valid está activo)
    // -------------------------------------------------------------------------
    always_comb begin
        mem_rdata = 32'h0; // Evita conflicto con ROM/RAM en el interconector
        if (mem_valid) begin
    ...
    always_ff @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            tx_start <= 1'b0; // default: pulso de un solo ciclo apagado
